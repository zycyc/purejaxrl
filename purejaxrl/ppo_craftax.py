        ...
        def _update_step(runner_state, unused):
            ...
            if config.get("WANDB_MODE", "disabled") == "online":
                def callback(info, loss):
                    ...
                    return_values = info["returned_episode_returns"][info["returned_episode"]]
                    timesteps = info["timestep"][info["returned_episode"]] * config["NUM_ENVS"]
                    scores = info["score"][info["returned_episode"]]
                    
                    # achievements
                    achievement_keys = [k for k in info.keys() if "Achievements" in k]
                    achievement_values = [info[k][info["returned_episode"]] for k in achievement_keys]

                    for t in range(len(timesteps)):
                        log_dict = {}
                        log_dict["time_step"] = timesteps[t]
                        log_dict["return_value"] = return_values[t]
                        log_dict["score"] = scores[t]
                        for k, v in zip(achievement_keys, achievement_values):
                            log_dict[k] = v[t]
